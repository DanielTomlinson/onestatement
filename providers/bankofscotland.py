...
def getBalance(security_info, account_number):
    ...
    r = session.get('https://online.bankofscotland.co.uk/personal/logon/login.jsp')
    ...
    # send first part of login request (username and password) to BoS
    payload = {
    ...
    r = session.post('https://online.bankofscotland.co.uk/personal/primarylogin', headers={'Referer': 'https://online.bankofscotland.co.uk/personal/logon/login.jsp', 'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/44.0.2403.89 Safari/537.36'}, data=payload)
    ...
    r = session.post('https://secure.bankofscotland.co.uk/personal/a/logon/entermemorableinformation.jsp', headers={'Referer': 'https://secure.bankofscotland.co.uk/personal/a/logon/entermemorableinformation.jsp', 'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/44.0.2403.89 Safari/537.36'}, data=payload)

    # check if we're on a promo page - if we are skip it
    promoMatch = re.search('<title>Bank of Scotland - Interstitial page</title>', r.text)
    if promoMatch:
        print ('Promo page spotted - skipping!')
        r = session.get('https://secure.bankofscotland.co.uk/personal/a/logon/interstitialpage.jsp?lnkcmd=continueLnk1&al=', headers={'Referer': 'https://secure.bankofscotland.co.uk/personal/a/logon/interstitialpage.jsp', 'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/44.0.2403.89 Safari/537.36'})
